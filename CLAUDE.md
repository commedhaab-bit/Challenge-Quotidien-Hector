...
Nouveau 4e sous-onglet du detail d'un groupe ("Raids", entre Defi et Ardoise).
Mini-defi spontane, mecanique volontairement plus simple qu'un defi classique :

- **Duree courte, choisie au lancement** (24h ou 48h, defaut 24h) : `deadline =
  createdAt + durationHours*3600*1000`, PAS un `endDate` choisi par calendrier
  (`<input type="date">`) comme un defi classique.
- **Plusieurs raids actifs en parallele autorises** dans un meme groupe -
  contrairement au defi classique (limite a un seul a la fois), aucune contrainte
  a faire respecter (ni cote client, ni cote regles) : simple liste
  (`groupDetailRaids`), pas de notion de "relevant" unique.
- **Enjeu FIXE et INVERSE** (pas de `stakeMode` a choisir) : `computeRaidSettlementPairs()`
  (logique pure, testee en isolation) - succes (cible atteinte) -> le CREATEUR doit
  une entree a chacun des autres (il offre la tournee) ; echec (echeance depassee
  sans atteindre la cible) -> c'est l'inverse, chacun des autres doit au createur.
  Jamais de classement N-way entre participants (contrairement a
  `computeSettlementPairs()`) : un pari binaire createur-vs-groupe.
- **Aucun joker** sur les raids (hors perimetre - mecanique volontairement plus
  rapide/simple qu'un defi collectif classique).

**Cote serveur** : reprend integralement le pipeline plafond+reglement instantane
des defis, adapte en 2 fonctions dediees pour eviter de complexifier le chemin
existant avec des branches conditionnelles "si c'est un raid" :
- `logRaidContribution` (Callable) : meme principe que
  `logGroupChallengeContribution` (resomme les participants, plafonne via
  `computeCreditedAmount()`, partagee), sans le multiplicateur Doublon.
- `settleRaidIfNeeded` : meme structure que `settleChallengeIfNeeded` (re-verifie
  sous transaction, ecrit `ledger`/rollups Hall of Fame/notifications), mais
  toujours sur le classement BRUT (jamais de jokers/Clutch Win pour les raids) et
  un seul champ `success` ecrit sur le raid. `closeExpiredGroupChallenges` balaie
  desormais AUSSI `collectionGroup('raids')` (filet de securite echeance,
  identique au principe des defis) - meme fonction planifiee, 2 balayages.
- Les entrees `ledger` d'un raid portent un champ `raidId` (au lieu de
  `challengeId` pour un defi classique) - meme forme de document sinon, donc
  apparaissent automatiquement dans l'Ardoise Globale (`groupDetailLedgerHistory`,
  triee sur `createdAt` uniquement) sans aucun changement cote rendu.
- Memes rollups Hall of Fame qu'un defi classique (`debtsOwed`/`totalVolume`/
  `challengesParticipated`) : un Raid Express compte pour ces statistiques comme un
  defi collectif a part entiere, juste plus court/spontane.

**Annulation** : meme mecanique que `cancelGroupChallenge` (transition `active` ->
`cancelled` gardee par `firestore.rules`, creator-only) - un raid annule disparait
simplement de la liste (filtre `status !== 'cancelled'`), jamais besoin d'un ecran
dedie puisque plusieurs raids peuvent de toute facon coexister.

**Regles Firestore** : `groups/{groupId}/raids/{raidId}` et sa sous-collection
`participants/{uid}` suivent EXACTEMENT la meme discipline que `challenges/*`
(participants create-only cote client, toute incrementation exclusivement via
`logRaidContribution`) - voir les commentaires de `challenges/*` dans
`firestore.rules` pour le detail du raisonnement, non redouble ici.

**Nouvel index** : `raids` (collectionGroup, `status`+`deadline`) ajoute a
`firestore.indexes.json`, meme raisonnement que l'index `challenges` existant
(prefixe `status` seul suffit a la requete `closeExpiredGroupChallenges`, le champ
`deadline` reste disponible pour une eventuelle requete future).

**`ensureMyParticipantDoc()` generalisee** : prend desormais directement la
REFERENCE de collection `participants` en argument (au lieu de `groupId`+
`challengeId`), pour etre reutilisable telle quelle par les defis ET les raids
